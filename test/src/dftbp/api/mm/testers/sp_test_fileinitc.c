...
void init_collective_variables(float *mermin_energy_total, float **gradients_total,
                               float **stress_tensor_total, float **gross_charges_total)
{
  int ii;

  *gradients_total = (float *) calloc(3 * MAX_ATOMS, sizeof(float));
  *stress_tensor_total = (float *) calloc(9, sizeof(float));
  *gross_charges_total = (float *) calloc(MAX_ATOMS, sizeof(float));
...
void update_collective_variables(int natom, float mermin_energy, float *gradients,
    float *stress_tensor, float *gross_charges, float *mermin_energy_total,
    float *gradients_total, float *stress_tensor_total, float *gross_charges_total)
{
  int ii;
...
int main()
{
  ...
  float coords_si2[3 * NR_OF_ATOMS_SI2] = {
  ...
  /* Coordinates in row major format, atomic units */
  float coords_h2o[3 * NR_OF_ATOMS_H2O] = {
  ...
  const float esp_locations[2*3] = {
    1.00000000000E+00,   0.00000000000E+00,   0.00000000000E+00,
    1.00000000000E+00,   0.10000000000E+00,   0.00000000000E+00,
  };

  float potential[2] = {0, 0};
  float potential_total[2] = {0, 0};

  float mermin_energy, mermin_energy_total;
  float *gradients, *gradients_total, *stress_tensor, *stress_tensor_total, *gross_charges, *gross_charges_total;
  int natom, natom0;
  int si2, ii, ij;
  ...
  for (ii = 0; ii < NR_ITER; ++ii) {
    ...
    /* Evaluate gradients */
    gradients = (float *) calloc(3 * natom0, sizeof(float));
    ...
    stress_tensor = (float *) calloc(9, sizeof(float));
    if(si2) {
    ...
    gross_charges = (float *) calloc(natom0, sizeof(float));
