...
#include <string.h>
#include <math.h>
#include "sp_dftbplus.h"
#include "sp_testhelpers.h"
 ...
 */
float distance(const float *aa, const float *bb)
{
 ...
 */
void calc_external_potential(int qmatoms, const float *qmcoords, int mmatoms,
                             const float *mmcoords, const float *mmcharges, float *extpot)
{
  const float *qmpos, *mmpos;
  float mmcharge, dist;
  int iatqm, iatmm;
...
*/
void calc_external_potential_grad(int qmatoms, const float *qmcoords, int mmatoms,
                                  const float *mmcoords, const float *mmcharges,
                                  float *extpotgrad)
{
  const float *qmpos, *mmpos;
  float mmcharge, dist, dist3;
  int iatqm, iatmm;
 ...
 */
void get_external_potential(void *refptr, float *dqatom, float *extpotatom)
{
  Context *cont;
 ...
 * potential (electron having a negative charge), you must invert its sign.
 */
void get_external_potential_grad(void *refptr, float *dqatom, float *extpotgrad)
{
  Context *cont;
...
{
  const float MM_COORDS[3 * NR_MM_ATOMS] = {
    -0.944863438887178, -9.44863438887178, 1.70075418999692,
     4.34637181888102,  -5.85815332110050, 2.64561762888410
  };

  const float MM_CHARGES[2] = {2.5, -1.9};

  const float QM_COORDS[3 * NR_QM_ATOMS] = {
  ...
  */
  cont->mmatoms = 1;
  cont->mmcoords = (float *) malloc(3 * cont->mmatoms * sizeof(float));
  memcpy(cont->mmcoords, MM_COORDS, 3 * cont->mmatoms * sizeof(float));
  cont->mmcharges = (float *) malloc(cont->mmatoms * sizeof(float));
  memcpy(cont->mmcharges, MM_CHARGES, cont->mmatoms * sizeof(float));

  cont->extpot = (float *) malloc(NR_QM_ATOMS * sizeof(float));
  ...
  cont->extpotgrad = (float *) malloc(3 * NR_QM_ATOMS * sizeof(float));
  calc_external_potential_grad(
      NR_QM_ATOMS, QM_COORDS, (NR_MM_ATOMS - cont->mmatoms), MM_COORDS + 3 * cont->mmatoms,
      MM_CHARGES + cont->mmatoms, cont->extpotgrad);

  cont->qmatoms = NR_QM_ATOMS;
  cont->qmcoords = (float *) malloc(3 * cont->qmatoms * sizeof(float));
  memcpy(cont->qmcoords, QM_COORDS, 3 * cont->qmatoms * sizeof(float));
}
...
int main()
{
  ...
  gradients = (float *) malloc(3 * NR_QM_ATOMS * sizeof(float));
  ...
  charges = (float *) malloc(NR_QM_ATOMS * sizeof(float));
